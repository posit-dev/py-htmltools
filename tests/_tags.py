...
test_that("Hanging commas don't break things", {
  expect_equal(as.character(tag_list("hi",)), "hi")
...
test_that("Head and singleton behavior", {
  ...
  result5 <- renderTags(tag_list(singleton(list(list("hello")))))
...
test_that("Indenting can be controlled/suppressed", {
  ...
  expect_identical(
    renderTags(tag_list(tags.div("a", "b")), indent = FALSE)$html,
    HTML("<div>\na\nb\n</div>")
  )
  expect_identical(
    format(tag_list(tags.div("a", "b")), indent = FALSE),
    "<div>\na\nb\n</div>"
  )
})
...
test_that("Latin1 and system encoding are converted to UTF-8", {
  ...
  expect_identical(Encoding(format(tag_list(latin1_str))), "UTF-8")
...
test_that("html render method", {
  ...
  tagFuncExt <- tagAddRenderHook(obj, function(x) {
    tagFunction(function() tag_list(x, tags.p("test")) )
  })
